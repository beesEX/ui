...
export default class AggregatedOrderBookTable extends React.PureComponent {
  ...
  componentDidMount() {
    ...
    ws.onmessage = (event) => {
      ...
      if (parsedData.type === 'ORDER_BOOK_EVENT' ) {
         ...
         */
        if (filledCompletely) this.removeVolumeByPrice(_side, index); // [Tung]: filledCompletely=true means, the the volumes (quantity and filled) at that price level should be decreased by quantity of the reason, but does not mean, that the volumes at that price level should be removed completely like this.
  ...
  progressMatches = (side, matches) => {
    ...
    for (let i = 0; i< matches.length; i++) {
      const { price, quantity, tradedQuantity } = matches[i]; // [Tung]: also destruct the 'filledCompletely' field of each match object
